...
// Run with: go test -tags live -count=1 ./internal/connectors/native/mangahub
func TestLiveResolveOnePiece(t *testing.T) {
	connector := NewConnector()
	ctx, cancel := context.WithTimeout(context.Background(), 60*time.Second)
	defer cancel()

	result, err := connector.ResolveByURL(ctx, "https://mangahub.io/manga/one-piece_142")
	if err != nil {
	...
	if result.LastUpdatedAt == nil {
		t.Fatalf("expected an update timestamp")
	}
...
func TestLiveSearchAndChapterURL(t *testing.T) {
	...
	url, err := connector.ResolveChapterURL(ctx, "https://mangahub.io/manga/one-piece_142", 1100)
	if err != nil {
